...
import { insertOfferSchema, insertCategorySchema, updateOfferSchema, insertBranchSchema, insertScreenAdSchema, insertScreenLocationSchema, insertScreenPricingOptionSchema, insertScreenBookingSchema, insertMerchantProfileSchema, insertInvoiceSchema, insertContactFormSchema, insertMerchantRegistrationSchema, screenLocations, screenBookings, invoices, users, merchantNotifications, campaignMedia, bookingLogs, contactForms, merchantRegistrations, subscriptionPlans, merchantSubscriptions, screenPricingOptions } from "@shared/schema";
import { z } from "zod";
import { db } from "./db";
...
const storage_multer = multer.diskStorage({
  destination: function (req, file, cb) {
    const uploadDir = file.fieldname === 'logo' ? 'uploads/logos' : 'uploads/screen-ads';
...
export function registerRoutes(app: Express): Server {
  ...
  app.post("/api/favorites", async (req, res) => {
    try {
      ...
      const alreadySaved = await storage.checkCustomerFavorite(phoneNumber, offerId);
      if (alreadySaved) {
  ...
  app.put("/api/business/profile", async (req, res) => {
    ...
    try {
      const allowedFields = [
        ...
        'businessCity', 'businessPhone', 'businessWebsite', 'businessWhatsapp',
        'businessInstagram', 'businessFacebook', 'businessSnapchat', 
        'businessX', 'businessTiktok', 'businessLogo'
      ];
  ...
  app.patch("/api/admin/businesses/:id/status", async (req, res) => {
    ...
    try {
      ...
      const updatedBusiness = await storage.updateBusinessStatus(businessId, isApproved);
  ...
  app.post("/api/admin/offers/:id/approve", async (req, res) => {
    ...
    try {
      ...
      const updatedOffer = await storage.updateOffer(id, { isApproved: true } as any);
      ...
      if (updatedOffer) {
        ...
        res.json({ message: "تم قبول العرض وإرسال إشعار للتاجر", offer: updatedOffer });
      } else {
        res.status(500).json({ message: "خطأ في تحديث العرض" });
      }
    } catch (error) {
  ...
  app.post("/api/admin/offers/:id/reject", async (req, res) => {
    ...
    try {
      ...
      const updatedOffer = await storage.updateOffer(id, { isApproved: false } as any);
      ...
      if (updatedOffer) {
        ...
        res.json({ message: "تم رفض العرض وإرسال إشعار للتاجر", offer: updatedOffer });
      } else {
        res.status(500).json({ message: "خطأ في تحديث العرض" });
      }
    } catch (error) {
  ...
  app.patch("/api/admin/plan-features/:planId/:featureId", async (req, res) => {
    ...
    try {
      ...
      const updatedPlanFeature = await storage.updatePlanFeature(planId, featureId, isIncluded, limit);
  ...
  app.post("/api/screen-ads", upload.single('mediaFile'), async (req, res) => {
    try {
      ...
      const { locationId, startDate, endDate, mediaType, duration, totalCost } = req.body;
  ...
  app.put("/api/admin/screen-locations/:id", async (req, res) => {
    try {
      ...
      const updated = await storage.updateScreenLocation(locationId, updateData);
  ...
  app.get("/api/invoices", async (req, res) => {
    try {
      ...
      const params = {
        merchantId: req.user.role === "admin" 
          ? (req.query.merchantId ? parseInt(req.query.merchantId as string) : undefined)
          : req.user.id,
        status: req.query.status as string,
        startDate: req.query.startDate ? new Date(req.query.startDate as string) : undefined,
        endDate: req.query.endDate ? new Date(req.query.endDate as string) : undefined,
  ...
  app.get("/api/analytics/dashboard", async (req, res) => {
    try {
      ...
      // Get total screens
      const totalScreensResult = await db.select({
        total: sql<number>`SUM(${screenLocations.numberOfScreens})`
      }).from(screenLocations).where(eq(screenLocations.isActive, true));
      ...
      }).from(screenBookings).where(eq(screenBookings.status, "approved"));
      ...
      const weeklyRevenueResult = await db.select({
        total: sql<string>`COALESCE(SUM(CAST(${invoices.totalAmount} AS DECIMAL)), 0)`
      }).from(invoices).where(
        and(
          eq(invoices.status, "paid"),
      ...
      const mostBookedLocations = await db.select({
      ...
      const mostActiveMerchants = await db.select({
  ...
  app.post("/api/campaign-media/:id/review", async (req, res) => {
    try {
      ...
      const [media] = await db.update(campaignMedia)
        .set({
          uploadStatus: status,
  ...
  app.post("/api/bookings/:id/logs", async (req, res) => {
    try {
      ...
      const [log] = await db.insert(bookingLogs)
        .values({
          bookingId,
          adminId: req.user.id,
          action,
  ...
  app.get("/api/merchant/campaign-history", async (req, res) => {
    try {
      ...
      const campaignStats = {
        ...
        completed: statsResult.find(s => s.status === "completed")?.count || 0,
  ...
  app.post("/api/merchant-register", upload.single('logo'), async (req, res) => {
    try {
  ...
  app.post("/api/admin/merchant-registrations/:id/approve", async (req, res) => {
    if (!req.isAuthenticated() || req.user.role !== 'admin') {
      return res.sendStatus(401);
    }
  ...
  app.get("/api/screen-locations", async (req, res) => {
    try {
      ...
      if (city && typeof city === 'string') {
        conditions.push(or(
          eq(screenLocations.city, city),
          eq(screenLocations.cityAr, city)
        ));
      }
      ...
      const locationsQuery = db
        .select({
          ...
          avgRating: sql<number>`COALESCE(AVG(${locationReviews.overallRating}), 0)`.as('avgRating'),
          reviewCount: sql<number>`COUNT(${locationReviews.id})`.as('reviewCount')
        })
        .from(screenLocations)
        .leftJoin(locationReviews, eq(locationReviews.locationId, screenLocations.id))
        .where(and(...conditions))
      ...
      if (minPrice && typeof minPrice === 'string') {
        ...
        filteredLocations = filteredLocations.filter(loc => 
          parseFloat(loc.dailyPrice) >= minPriceNum
        );
      }
      ...
      if (maxPrice && typeof maxPrice === 'string') {
        ...
        filteredLocations = filteredLocations.filter(loc => 
          parseFloat(loc.dailyPrice) <= maxPriceNum
        );
      }
      ...
      if (minRating && typeof minRating === 'string') {
        ...
        filteredLocations = filteredLocations.filter(loc => loc.avgRating >= minRatingNum);
      }
  ...
  app.get("/api/subscription/plans", async (req, res) => {
    try {
      const result = await db.execute(sql`SELECT * FROM subscription_plans WHERE is_active = true ORDER BY sort_order`);
  ...
  app.post("/api/merchant/subscribe", async (req, res) => {
    ...
    try {
      ...
      // Check if plan exists
      const planResult = await db.execute(sql`SELECT * FROM subscription_plans WHERE id = ${planId}`);
  ...
  app.get("/api/merchant/subscription", async (req, res) => {
    ...
    try {
      ...
      if (result.rows.length === 0) {
        // Return free plan as default
        const freePlanResult = await db.execute(sql`SELECT * FROM subscription_plans WHERE name = 'Free' LIMIT 1`);
        if (freePlanResult.rows.length > 0) {
          ...
          return res.json({
            ...
            plan: {
              ...
              offerLimit: freePlan.offer_limit,
      ...
      res.json({
        ...
        plan: {
          ...
          offerLimit: subscription.offer_limit,
  ...
  app.post("/api/reviews/submit", async (req, res) => {
    ...
    try {
      ...
      const ratings = [overallRating, visibility, performance, traffic, valueForMoney];
      if (ratings.some(rating => rating < 1 || rating > 5)) {
        return res.status(400).json({ error: "التقييمات يجب أن تكون بين 1 و 5" });
      }
      ...
      if (bookingId) {
        ...
        if (bookingResult.rows.length === 0) {
          return res.status(403).json({ error: "يمكنك فقط تقييم المواقع التي حجزتها وأكملت حملاتها" });
        }
      }
      ...
      if (existingReview.rows.length > 0) {
        return res.status(400).json({ error: "لقد قمت بتقييم هذا الحجز مسبقاً" });
      }
      ...
      await db.execute(sql`
        INSERT INTO screen_reviews (
          ...
          ${performance}, ${traffic}, ${valueForMoney}, ${title}, ${comment}, ${bookingId ? true : false}
        )
      `);
  ...
  try {
    ...
    if (existingCategories.length === 0) {
      const defaultCategories = [
        { name: "Restaurants", nameAr: "المطاعم", emoji: "🍽️", slug: "restaurants", description: "مطاعم وأطعمة" },
        { name: "Travel & Tourism", nameAr: "السفر والسياحة", emoji: "✈️", slug: "travel-tourism", description: "سفر وسياحة" },
        { name: "Cafes", nameAr: "المقاهي", emoji: "☕", slug: "cafes", description: "مقاهي ومشروبات" },
        { name: "Clinics & Beauty", nameAr: "العيادات والتجميل", emoji: "💆‍♀️", slug: "clinics-beauty", description: "عيادات وتجميل" },
        { name: "Online Stores", nameAr: "المتاجر الإلكترونية", emoji: "🛍️", slug: "online-stores", description: "متاجر إلكترونية" },
        { name: "Education", nameAr: "الدورات والتعليم", emoji: "🧠", slug: "education", description: "دورات وتعليم" },
        { name: "Today's Deals", nameAr: "عروض اليوم", emoji: "🔥", slug: "todays-deals", description: "عروض محدودة الوقت" },
      ];
  ...
  try {
    ...
    const plans = [
      ...
      },
      {
        ...
        advancedReports: false,
        displayOrder: 2,
        features: ["Up to 5 bookings", "Premium support", "Priority booking", "Email notifications"]
      },
      {
        name: "Premium",
        nameAr: "مميز", 
        ...
        displayOrder: 3,
        features: ["Unlimited bookings", "Premium support", "Advanced reports", "Analytics dashboard", "API access", "Priority support"]
      }
    ];
